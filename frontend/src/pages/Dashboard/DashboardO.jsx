/**
 * Dashboard: default home after login; question list, URL-driven search.
 * States: questions, searchQuery, searchMode, isLoading, error
 * Data: getQuestions({ search }) for keyword, searchQuestionsSemantic(query) for semantic
 */
...
import { PenSquare, Library, BookOpen } from "lucide-react";
...
export default function Dashboard() {
  ...
  // ── React to URL params (driven by Navbar) ─────────────────────────────────
  useEffect(() => {
        ...
        <div className={styles.feed__header}>
          <div>
            ...
            <p className={styles.feed__sub}>
              ...
              {searchQuery &&
                searchMode === "keyword" &&
                `Keyword results for "${searchQuery}"`}
              {!searchQuery &&
          ...
          <button
            type="button"
            className={`${styles.feed__sortBtn} ${sortMode === "newest" ? styles["feed__sortBtn--active"] : ""}`}
            onClick={() =>
